...
parser.add_argument('--img_size', type=int, nargs='+', default=[128,128,128], help='3D ROI size, e.g., [128,128,128]') #500
...
parser.add_argument('--eval_step', type=int, default=200, help='Per steps to perform validation') #500
...
#把耗时的 transform 提前算好并存在内存里，后面训练直接用缓存后的数据
...
# macs, params = get_model_complexity_info(model, (args.n_channels, args.img_size[0], args.img_size[1], args.img_size[2]), as_strings=True, print_per_layer_stat=True, verbose=True)
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with autocast(enabled=False):
            ...
            if isinstance(p, torch.Tensor):
                ...
                P = list(p)  # 把 tuple 拆成 list，每个元素应该是一个 Tensor
            else:
                raise TypeError(f"Unexpected model output type: {type(p)}")
            if out_classes == 9:       
                y[y==5] = 0
...
def validation_save(epoch_iterator_val): 
    ...
    with torch.no_grad():
        for step, batch in enumerate(epoch_iterator_val):
            ...
            _, _, h, w, d = val_labels.shape
            target_shape = (h, w, d)
            # output_folder = os.path.join(args.results, args.network)
...
# "8": "arota", "9": "postcava", "10": "pancreas", "11": "right adrenal gland", "12": "left adrenal gland", "13": "duodenum", "14": "bladder", "15": "prostate/uterus"
