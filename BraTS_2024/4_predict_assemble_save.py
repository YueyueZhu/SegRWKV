...
class BraTSTrainer(Trainer):
    ...
    def define_model(self):
        ...
        model = SegRWKV(4, 5, kn=(28, 36, 48, 64, 80), FMU='sub')

        model_path = "./data/3D_parameter/SegRWKV_BraTS_2024.pth"
        ...
        save_path = "./data/3D_results/brats24"+"/"+file_name
        os.makedirs(save_path, exist_ok=True)
        save_visual_path = "./data/3D_visual_results/brats24"+"/"+file_name
    ...
    def convert_labels_dim0(self, labels):
        # ET = 3， NETC = 1， SNFH = 2， RC = 4， tumor core = 1+3，whole tumor = 1+2+3
        result = [labels == 0, labels == 3, labels == 1, labels == 2, labels == 4, (labels == 1) | (labels == 3), (labels == 1) | (labels == 3) | (labels == 2)]
    ...
    def validation_step(self, batch):
        ...
        voxel_spacing = [1, 1, 1]
        case_name = properties['name'][0]
        raw_data_dir = "./data/BraTS2024-BraTS-GLI-TrainingDat/"
...
def save_topk_slices(pred, label, case_name, T1_bg, top_k=4, step=10, save_dir="./data", dpi=400):
    ...
    pred, label, T1_bg = to_np(pred), to_np(label), to_np(T1_bg)  # pred/label: (C,Z,H,W)
    C, Z, H, W = pred.shape
    ...
    def collapse_labels(multi_hot):
        ...
        # 优先级：ET(3) > TC(1) > WT(2) > BG(0)
        final[et > 0] = 3
        final[(tc > 0) & (final == 0)] = 1
        final[(wt > 0) & (final == 0)] = 2
        return final

    pred_labels = collapse_labels(pred)   # (Z,H,W)
    gt_labels   = collapse_labels(label)  # (Z,H,W)
    ...
    for rank, z in enumerate(idx):
        ...
        # plt.imshow(img_gt)
        # plt.axis("off")
        # plt.savefig(f_gt, bbox_inches="tight", pad_inches=0)
        # plt.close()
        ...
        #  T1 
        # plt.figure(figsize=(4,4), dpi=dpi)
        # plt.imshow(slice_img_crop, cmap='gray')
        # plt.axis("off")
        # plt.savefig(f_t1_crop, bbox_inches="tight", pad_inches=0)
        # plt.close()
...
if __name__ == "__main__":
    ...
    # pkill 4_predict_assemble_save.py
